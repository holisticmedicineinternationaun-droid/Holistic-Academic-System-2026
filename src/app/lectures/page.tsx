...
// Scholar Jurist Voice Content Generator
const generateScholarJuristContent = (topic: string, sectionTitle: string) => {
    return `نستفتح بالذي هو خير، وفي رحلتنا لفك شفرة هذا العرض العضوي المسمى بـ (${topic})، نجد أن الأبعاد الستة تتشابك في نسيجٍ واحدٍ لا يقبل التجزئة. فالمبحث الذي بين أيدينا اليوم، والموسوم بـ (${sectionTitle})، ليس مجرد سردٍ للحقائق الصماء، بل هو تأصيلٌ وجداني وعلمي يربط بين ميكانيكا الخلية ونبض الروح. إننا حين نتأمل الأسرار المكتشفة في هذا المسار، ندرك أن ميزان المجلس التاسع يفرض علينا دقةً تفوق الوصف، حيث يسلمنا كل معنى إلى ما يليه بشغفٍ معرفي متصل، مؤكدين أن العلم في محراب الطب الشمولي هو عبادةٌ وتفكر، يستوجب منا استحضار هيبة الدليل وقوة الاستنباط.`;
};
...
const generateLectureStructure = (title: string, objectives: string, audience: string) => {
    return {
        ...
        objectives,
        audience,
        basmala: "بسم الله الرحمن الرحيم - نبتدئ بالحمد والثناء طلباً للفتح والبركة في هذا المورد العلمي.",
        intro: `مقدمة المحاضرة: صياغة احترافية تعكس هيبة العلم، تستهدف (${audience})، وتهدف إلى (${objectives})، مع دمج الأسرار البحثية والمنهج المدمج المقارن.`,
        chapters: [
            {
                id: 'ch1',
